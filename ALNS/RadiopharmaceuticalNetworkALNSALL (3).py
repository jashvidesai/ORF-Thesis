...
# print function for only the used vehicles fixed costs
def print_vehicle_routes_with_costs(vehicles, c):
    ...
    for vehicle in vehicles:
        ...
        if is_used:
            # Add fixed cost only for used vehicles
    ...
    print("\nTOTAL COST BREAKDOWN (Used Vehicles Only):")
    print(f"  Fixed Cost (Used Vehicles): {round(total_fixed_cost, 2)}")
...
def compute_total_cost(solution, c, fixed_costs):
    ...
    for vehicle in solution:
        # Only include vehicles that are used (more than just depot)
...
# checks if the singleton cluster is feasible
...
def deduplicate_final_clusters(clusters):
    # Tracks clusters that we have already seen
    seen = set()
    unique_clusters = []

    for cluster in clusters:
        # Use Vehicle ID and the exact route tuple as a key
        ...
        # If unique then add to the list!
        if key not in seen:
            seen.add(key)
...
# Converts the clustes to a "vehicle" type format to be future-operator friendly
def convert_clusters_to_vehicle_solution(
    ...
    new_solution = [] # The list to hold the newly created vehicle solution
    used_vehicle_ids = set() # Track vehicles that have already been added

    for cluster in final_clusters:
        # Get the vehicle assigned to this cluster
        vehicle = cluster.get('assigned_vehicle')
        if vehicle is None:
            print(f"Cluster {cluster['customers']} has no assigned vehicle.")
            continue # Skip clusters without a vehicle assignment

        # Skip already used vehicles to avoid duplicates in the solution
        ...
        # Assign the formatted route to the vehicle and the cluster
...
# Print the final routes and calculate the total cost of the routes from step above
def print_final_routes_and_compute_total_cost(clusters, c_matrix):
    ...
    for idx, cluster in enumerate(clusters):
        ...
        total_travel_cost += travel_cost
        total_fixed_cost += vehicle.cost  # Only add fixed cost for used vehicles
        used_vehicle_ids.add(vehicle.vehicle_id)
    ...
    print(f"  Fixed Vehicle Cost (used only):   {round(total_fixed_cost, 2)}")
...
# Convert the list of vehicles into a list of clusters
...
def simulate_capacity_and_insert_depot(final_cleaned_clusters, deliveries, pickups, vehicle_caps, t_matrix, a, b, s):
    updated_clusters = [] # The list to hold the updated clusters
    depot_node = 0 # Depot is always node 0
    ...
    for cluster in final_cleaned_clusters:
        ...
        if not route or not vehicle:
            ...
            continue # Skip clusters without route or vehicle
...
def savings_insertion(
    ...
    # Step 15: Update The Updated Delivery/Pickup Values based on what's been served
    updated_deliveries = deepcopy(original_deliveries)
    updated_pickups = deepcopy(original_pickups)
    ...
    # Final convert the recent output for that good format; function in Step 14 not sufficient
...
def print_final_routes_and_compute_total_cost(routes_or_clusters, c_matrix):
    ...
    for idx, item in enumerate(routes_or_clusters):
        ...
        if isinstance(item, dict):
            ...
            vehicle = item
            route = vehicle.route  # Use full route info with delivery/pickup/time
        ...
        route_str = " → ".join("Depot" if stop[0] == 0 else str(stop[0]) for stop in route)
        print(f"   Vehicle {vehicle.vehicle_id} | Route: {route_str}")

        # Compute travel cost using actual nodes
        travel_cost = 0
        for i in range(len(route) - 1):
            from_node = route[i][0]
            to_node = route[i + 1][0]
...
# checks whether the vehicles in the solution are able to fulfill the delivery and pickup requirements
def detailed_capacity_check_with_logging(
    ...
    report = [] # List to store detailed log of the feasibility checks for each vehicle
    overall_feasible = True # Flag to track if the whole solution is feasible
    ...
    # Check the feasibility for each vehicle
    for vehicle in solution:
        ...
        i = 1
        # Iterate through the route and check if each stop can be served within vehicle capacity
        while i < len(route):
            node_id = route[i][0]
            ...
            # This accounts for future split deliveries
            ...
            # Log the status of the current node (whether it was fully served or not)
            ...
            if delivery_done < delivery_needed or pickup_done < pickup_needed:
                if will_be_visited_again:
                    status += f"⚠️ Partial (will be served again)" # If not fully served and will be revisited, it's status is still ok
...
# Checks if all routes are time feasible
...
def update_operator_weights_and_scores(rem_op, ins_op, outcome, removal_scores, insertion_scores):
    if outcome == "best":
        removal_scores[rem_op] += 4
        insertion_scores[ins_op] += 4
    elif outcome == "accept":
        ...
        removal_scores[rem_op] += 0
        insertion_scores[ins_op] += 0
...
def ALNS(initial_solution, original_deliveries, original_pickups, c, t, Q, a, b, s, distances, fixed_costs, T=0.1, MAX_ITERATIONS=50, STAGNATION_LIMIT=5):
    ...
    # basically initializes the operators for tracking history
    ...
    for iteration in range(MAX_ITERATIONS):
        ...
        try:
            ...
            # Log current weights/scores anyway but they don't change!
        ...
        if not feasible:
            ...
            # Track weights as constant and mark as infeasible
            for op in removal_operators:
                removal_weights_history[op].append(removal_weights[op])
                removal_scores_history[op].append(removal_scores[op])
            for op in insertion_operators:
        ...
        if not time_feasible:
            outcome = "infeasible"
            stagnation_counter += 1
            # Again, this tracks weights as constant and mark as infeasible
            for op in removal_operators:
                removal_weights_history[op].append(removal_weights[op])
                removal_scores_history[op].append(removal_scores[op])
            for op in insertion_operators:
...
plt.plot(cost_history, marker='o', label='Calculated Solution Cost', linewidth=2)
...
plt.plot(insertion_weights_history["parallel"], label="Parallel", linewidth=2, linestyle='--', color='blue', alpha=0.7)
plt.plot(insertion_weights_history["regret"], label="Regret", linewidth=2, linestyle='-', color='orange')
plt.plot(insertion_weights_history["savings"], label="Savings", linewidth=2, color='green')
...
"""This is a simple time check for the route generated by ALNS."""
...
def check_all_routes_time_feasible(vehicles, t_matrix, a, b, s):
    ...
    for vehicle in vehicles:
        ...
        for i in range(1, len(route)):
            prev_node = route[i - 1][0]  # Extract node ID of previous node
            curr_node = route[i][0]      # Extract node ID of current node

            # Calculate the arrival time at the current node
            arrival_time += t_matrix[prev_node][curr_node]
            if arrival_time < a[curr_node]:
                arrival_time = a[curr_node]  # Adjust if arrival is before the earliest time
            if arrival_time > b[curr_node]:
                route_feasible = False
                all_feasible = False
                report.append(
            ...
            arrival_time += s[curr_node]  # Add the service time at the node

            # Print the arrival time for each node
            print(f"Node {curr_node}: Arrival Time = {arrival_time:.2f}, Time Window = [{a[curr_node]}, {b[curr_node]}]")
...
# Example route and inputs (simplified for illustration)
...
# Define the updated vehicles
vehicles = [
    Vehicle(6, [
        (0, 0, 0, 0.0), (7, np.int64(15), np.int64(5), 0.0), (5, np.int64(15), np.int64(11), 0.0),
        (33, np.int64(3), np.int64(14), 0.0), (12, np.int64(11), np.int64(13), 0.0),
        (35, np.int64(13), np.int64(14), 0.0), (28, np.int64(6), np.int64(3), 0.0),
        (11, np.int64(10), np.int64(11), 0.0), (0, 0, 0, 0.0), (8, np.int64(14), np.int64(10), 0.0),
        (26, np.int64(2), np.int64(4), 0.0), (19, np.int64(14), np.int64(10), 0.0),
        (20, np.int64(1), np.int64(11), 0.0), (27, np.int64(8), np.int64(2), 0.0),
        (30, np.int64(2), np.int64(4), 0.0), (44, np.int64(2), np.int64(4), 0.0),
        (32, np.int64(4), np.int64(9), 0.0), (38, np.int64(12), np.int64(12), 0.0),
        (3, np.int64(3), np.int64(13), 0.0), (42, np.int64(8), np.int64(5), 0.0),
        (41, np.int64(4), np.int64(3), 0.0), (24, np.int64(1), np.int64(2), 0.0),
        (40, np.int64(6), np.int64(13), 0.0), (0, 0, 0, 0.0)
    ]),
    Vehicle(2, [
        (0, 0, 0, 0.0), (2, np.int64(5), np.int64(7), 0.0), (29, np.int64(6), np.int64(12), 0.0),
        (39, np.int64(12), np.int64(9), 0.0), (21, np.int64(15), np.int64(5), 0.0),
        (31, np.int64(3), np.int64(9), 0.0), (17, np.int64(4), np.int64(8), 0.0),
        (16, np.int64(15), np.int64(2), 0.0), (25, np.int64(13), np.int64(15), 0.0),
        (18, np.int64(4), np.int64(4), 0.0), (0, 0, 0, 0.0), (37, np.int64(15), np.int64(10), 0.0),
        (13, np.int64(12), np.int64(14), 0.0), (4, np.int64(2), np.int64(12), 0.0),
        (14, np.int64(3), np.int64(9), 0.0), (6, np.int64(10), np.int64(5), 0.0),
        (10, np.int64(13), np.int64(7), 0.0), (43, np.int64(2), np.int64(12), 0.0),
        (15, np.int64(14), np.int64(8), 0.0), (23, np.int64(6), np.int64(6), 0.0),
        (0, 0, 0, 0.0)
    ]),
    Vehicle(3, [
        (0, 0, 0, 0.0), (1, np.int64(8), np.int64(2), 0.0), (9, np.int64(7), np.int64(13), 0.0),
        (22, np.int64(13), np.int64(11), 0.0), (34, np.int64(9), np.int64(14), 0.0),
        (36, np.int64(5), np.int64(4), 0.0), (0, 0, 0, 0.0)
    ])
]

# Run the feasibility check
all_feasible, report = check_all_routes_time_feasible(vehicles, t_matrix_np, a, b, s)
